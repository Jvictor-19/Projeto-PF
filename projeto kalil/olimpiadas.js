...
const QuantasAtletasfemininasparticiparam = (lista,cidade) => 
      lista.filter((x) => cidade.includes(x.City))
      .filter((x) => x.Sex == "F")
      .map((x) => x.Name)
...
const primeiroPaisAmericaLatina = (lista) => lista.filter((x) => x.City == "Cidade do Mexico") ? "México" : "Não foi encontrada"
...
lerCSV('arquivo.csv')
    .then((dados) => {
        ...
        const VezesQueaCidadeSediou = quantasVezesForamSediadas(dados, cidadesediou)
        console.log(`${cidadesediou} sediou ${VezesQueaCidadeSediou} vezes.`)
        const PaisAmericaLatina = primeiroPaisAmericaLatina (dados)
        console.log(`O primeiro pais Da America Latina a sediar o jogos de verão foi o ${PaisAmericaLatina} em 1968 com sua cidade sede a Cidade do México.`)
    })
    .catch((error) => {
        console.error('Erro ao ler o arquivo CSV:', error)
    });
